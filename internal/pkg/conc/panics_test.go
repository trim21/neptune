...
func ExampleCatcher_callers() {
	...
	// Output:
	// neptune/internal/pkg/conc.(*Catcher).tryRecover
	// runtime.gopanic
	// neptune/internal/pkg/conc_test.ExampleCatcher_callers.func1
	// neptune/internal/pkg/conc.(*Catcher).Try
	// neptune/internal/pkg/conc_test.ExampleCatcher_callers
	// testing.runExample
	// testing.runExamples
	// testing.(*M).Run
	// main.main
...
func TestCatcher(t *testing.T) {
	...
	t.Run("not error", func(t *testing.T) {
		...
		require.NoError(t, errors.Unwrap(recovered.AsError()))
	})
