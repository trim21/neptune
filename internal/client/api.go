...
func (c *Client) GetTorrentList(keys []string) TorrentList {
	...
	torrents := make([]MainDataTorrent, len(c.downloads))
	...
	for i, d := range c.downloads {
		...
		torrents[i] = MainDataTorrent{
			...
			Completed:            info.BytesCompleted,
			TotalLength:          info.TotalLength,
			SelectedSize:         info.SelectedSize,
			...
			TrackerErrors:        info.TrackerErrors,
			TotalSeeding:         info.TotalSeeding,
			TotalDownloading:     info.TotalDownloading,
			ConnectedSeeding:     info.ConnectedSeeding,
			ConnectedDownloading: info.ConnectedDownloading,
		}
	}
...
func (c *Client) AddTorrent(raw []byte, m *metainfo.MetaInfo, info meta.Info, downloadPath string, tags []string, custom map[string]string, selectedFiles []int, skipHashCheck bool) error {
	...
	dir := filepath.Join(c.session.TorrentPath, h[:2], h[2:4])
	...
	slices.SortFunc(c.downloads, func(a, b *Download) int {
		return bytes.Compare(a.InfoHashBytes(), b.InfoHashBytes())
	})
...
func (c *Client) GetTorrentFiles(h metainfo.Hash) []TorrentFile {
	...
	files := d.Files()
	results := make([]TorrentFile, len(files))
	for i, f := range files {
...
func (c *Client) GetTorrentPeers(h metainfo.Hash) []APIPeers {
	...
	peerInfos := d.PeerInfos()
	results := make([]APIPeers, len(peerInfos))
	for i, pi := range peerInfos {
		results[i] = APIPeers{
			Address:      pi.Address,
			Client:       pi.Client,
			Progress:     pi.Progress,
			DownloadRate: pi.DownloadRate,
			UploadRate:   pi.UploadRate,
			IsIncoming:   pi.IsIncoming,
